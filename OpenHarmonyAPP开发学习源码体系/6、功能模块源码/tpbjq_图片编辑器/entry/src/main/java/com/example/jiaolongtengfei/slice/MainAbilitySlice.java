package com.example.jiaolongtengfei.slice;

import com.example.jiaolongtengfei.ResourceTable;
...
public class MainAbilitySlice extends AbilitySlice {
    ...
    private void initView() {
        if (findComponentById(ResourceTable.Id_jltfwhirl_image) instanceof Button) {
            whirlImageBtn = (Button) findComponentById(ResourceTable.Id_jltfwhirl_image);
        }
        if (findComponentById(ResourceTable.Id_jltfcrop_image) instanceof Button) {
            cropImageBtn = (Button) findComponentById(ResourceTable.Id_jltfcrop_image);
        }
        if (findComponentById(ResourceTable.Id_jltfscale_image) instanceof Button) {
            scaleImageBtn = (Button) findComponentById(ResourceTable.Id_jltfscale_image);
        }
        if (findComponentById(ResourceTable.Id_jltfmirror_image) instanceof Button) {
            mirrorImageBtn = (Button) findComponentById(ResourceTable.Id_jltfmirror_image);
        }
        if (findComponentById(ResourceTable.Id_jltfimage) instanceof Image) {
            image = (Image) findComponentById(ResourceTable.Id_jltfimage);
        }
    ...
    private class ButtonClick implements Component.ClickedListener {
        ...
        public void onClick(Component component) {
            ...
            switch (btnId) {
                case ResourceTable.Id_jltfwhirl_image:
                    // 旋转图片
                    whirlCount++;
                    isCorp = false;
                    isScale = false;
                    isMirror = false;
                    imagePixelMap = getPixelMapFromResource(ResourceTable.Media_shici1);
                    image.setPixelMap(imagePixelMap);
                    break;
                case ResourceTable.Id_jltfcrop_image:
                    // 剪裁图片
                    whirlCount = 0;
                    isCorp = !isCorp;
                    isScale = false;
                    ...
                    image.setPixelMap(imagePixelMap);
                    break;
                case ResourceTable.Id_jltfscale_image:
                    // 缩放图片
                    whirlCount = 0;
                    isCorp = false;
                    isScale = !isScale;
                    ...
                    image.setPixelMap(imagePixelMap);
                    break;
                case ResourceTable.Id_jltfmirror_image:
                    // 镜像图片
                    whirlCount = 0;
                    isCorp = false;
                    isScale = false;
